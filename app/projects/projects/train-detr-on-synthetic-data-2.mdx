---
title: 'Train a Vision Transformer on Synthetic Data, Part 2: Train and Evaluate your model.'
publishedAt: '2025-12-14'
summary: 'Explore the enduring debate between using spaces and tabs for code indentation, and why this choice matters more than you might think.'
---

_This is an updated version of the blog I made years ago on generating synthetic data with Blender to train a YOLO model, if you're looking for that blog, you can find it [here](https://federicoarenasl.github.io/sdg-engine/)._
...
This tutorial will walk you through how to train a Vision Transformer using your own synthetic data for object detection. We'll use Blender to generate the data, and train a DEtection TRansformer (DETR) model with help from my minimal package, `sdg-engine`. The training and validation will be done with the HuggingFace `transformers` and `datasets` libraries, using real validation data from the open source "BOP: Benchmark for 6D Object Pose Estimation" [1].

This tutorial is divided in two parts:
1. [**Part 1: Generate data using Blender**](/projects/train-detr-on-synthetic-data-1): this part focuses on setting up your Blender scene, your `sdg-package`, generating your training data, and pushing your `train` and `validation` datasets into the HuggingFace Hub.
2. [**Part 2: Train DETR on Synthetic Data**](/projects/train-detr-on-synthetic-data-2): this part focuses on taking your `train`, and `validation` datasets and training DETR  using the HuggingFace `transformers` library.
...
## Prerequisites
Before we start, you'll need an NVIDIA or Apple Silicon GPU in order to train DETR.
...
The Homebrew dataset has a multitude of scenes with different objects rotating on a platform, and for simplicity we'll train our model to detect objects in only one of their scenes -- scene No. 3 -- shown below:
...
## iv. Train DETR using the `transformers` library
Now that we have both our evaluation and our training datasets somewhere safely in the HuggingFace Hub, we are ready to use them to train our DEtection TRansformer!
...
### Augment the data
Data augmentations are a common thing in any ML training pipeline, they let us incentivize generalization in our model via data transformations in the training dataset. We'll prepare these using the albumentations library. This library is awesome, you can `albumentations.Compose` a series of augmentations which will then be applied to each of the datasets.

Let's introduce transformations in our training dataset to make the model more robust to rotational variations, brightness and constrast variations, noise variations, and other variations in the images:
...
Awesome! We now have our `train_dataset_transformed` and our `val_dataset_transform` ready to use during training and validation.
...
The collate function combines individual samples into batches for training. It ensures all images are properly padded and formatted:
...
To track our training metrics and visualize results, we'll use Weights & Biases. Set up logging with a unique run identifier:
...
With our datasets prepared and evaluation metrics configured, we can now initialize the DETR model and set up the training parameters.
...
Next, define the training hyperparameters. These values control the learning process, batch sizes, evaluation frequency, and logging:
...
Now create the `Trainer` object that orchestrates the training loop, combining the model, datasets, and configuration:
...
The model will train for 10,000 steps, evaluating on the real-world validation set every 100 steps and logging metrics to Weights & Biases.

#### iv-4. Training run
After running the training for 10,000 steps, we can observe the model's learning progression through several key metrics.

The training loss shows a steady decline throughout the training process, indicating that the model is effectively learning from the synthetic data:

<Image
  src="/train_loss.svg"
  alt="Training loss curve showing steady decline over 10,000 steps"
  width={500}
  height={315}
/>
...
The mean Average Precision (mAP) metric steadily improves during training, demonstrating that the model's detection accuracy is increasing:

<Image
  src="/eval_map.svg"
  alt="Mean Average Precision (mAP) metric progression during training"
  width={500}
  height={315}
/>

Here's a demonstration of the trained model detecting objects in the real-world Homebrew dataset:

<Image
  src="/trained_demo.gif"
  alt="Trained DETR model detecting objects in the Homebrew dataset"
  width={500}
  height={315}
/>
...
In this tutorial, we successfully trained a DETR model using entirely synthetic data generated from Blender and evaluated it on real-world images from the BOP Homebrew dataset. The key takeaways include:

- **Synthetic-to-real transfer is feasible**: Models trained on synthetic data can generalize to real-world scenarios when the synthetic data captures sufficient visual diversity.
...
- **Fine-tune on real data**: Use the synthetic-trained model as a starting point and fine-tune it with a small amount of real-world labeled data.
